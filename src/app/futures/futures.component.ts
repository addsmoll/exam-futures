...
import {concatMap, delay, map, Observable, of, range, Subject, takeUntil, timer} from 'rxjs';
...
export class FuturesComponent implements OnInit, OnDestroy
{
  ...
  data: any;
  currentSeriesRange: number;
  currentSeries: IRange =  { label:'5 min', value: 1 }
    ...
    ngOnInit(): void
    {
      ...
      this.timer5Sec
        ...
        this._futuresService.getData()
          .pipe(takeUntil(this._unsubscribeAll)).subscribe((res) => console.log('value', res))
      ...
      this.timer5Min
        ...
        .subscribe(val => {
          ...
          this._futuresService.changeSeries(this.currentSeries.value + 1)
            .pipe(takeUntil(this._unsubscribeAll)).subscribe((res) => console.log('value', res))
...
/*      window['Apex'] = {
  ...
  onSeriesRangeChange(range: IRange) {
    this.currentSeries = range
    this.compareFuturesBySeries(this.currentSeries.value)
  }
